...
class BorderlineSMOTE1D:
    ...
    def fit_resample(self, X, y):
        ...
        for _ in range(n_to_generate):
            ...
            minority_nbrs = NearestNeighbors(
                n_neighbors=min(self.k_neighbors, len(minority_samples))
            )
        ...
        synthetic_samples = np.array(synthetic_samples).reshape(
...
def apply_smote_balancing(train_dataset):
    ...
    print(
        f"  Non-target (0): {len(y)-np.sum(y)} samples ({(len(y)-np.sum(y))/len(y)*100:.1f}%)"
    )
    ...
    balanced_targets = [
        ("target" if label == 1 else "nontarget", i)
    ...
    print(
        f"  Target (1): {np.sum(y_balanced)} samples ({np.sum(y_balanced)/len(y_balanced)*100:.1f}%)"
    )
    print(
        f"  Non-target (0): {len(y_balanced)-np.sum(y_balanced)} samples ({(len(y_balanced)-np.sum(y_balanced))/len(y_balanced)*100:.1f}%)"
    )
...
def select_optimal_time_window(data, targets, fs=250):
    ...
    nontarget_indices = [
        i for i, t in enumerate(targets) if t[0] == "nontarget"
    ]

    if len(target_indices) == 0 or len(nontarget_indices) == 0:
        print(
            "Предупреждение: недостаточно данных для анализа временного окна"
        )
        start_idx = 250
        end_idx = 500
