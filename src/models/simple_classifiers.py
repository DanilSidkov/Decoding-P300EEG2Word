    ...
    def _create_classifier(self, classifier_type, params):
        ...
        if classifier_type == "svm":
            ...
            return RandomForestClassifier(
                n_estimators=100, max_depth=10, random_state=42, **params
            )
        elif classifier_type == "logistic_regression":
            return LogisticRegression(
                C=1.0, random_state=42, max_iter=1000, **params
            )
...
def compare_classifiers(X_train, y_train, X_val, y_val):
    ...
    best_name = max(
        results.keys(), key=lambda x: results[x]["val_balanced_accuracy"]
    )
    best_model = results[best_name]["model"]

    print(f"\n=== ЛУЧШИЙ КЛАССИФИКАТОР: {best_name} ===")
    print(
        f"Validation balanced accuracy: {results[best_name]['val_balanced_accuracy']:.4f}"
    )
