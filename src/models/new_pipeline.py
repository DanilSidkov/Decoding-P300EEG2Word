...
from models.eeg import DeepConvNet
from models.eegnet1d import OptimizedEEGNet1D_v2
from models.smote import (
    apply_time_window,
    select_optimal_time_window,
)
from models.train_model import print_model_parameters, test_model, train_model
...
def improved_pipeline(
    ...
    try:
        ...
        if method == "csp":
            ...
            print(
                f"Train - Nontarget: {np.sum(y_train == 0)}, Target: {np.sum(y_train == 1)}"
            )
            print(
                f"Val - Nontarget: {np.sum(y_val == 0)}, Target: {np.sum(y_val == 1)}"
            )
            print(
                f"Test - Nontarget: {np.sum(y_test == 0)}, Target: {np.sum(y_test == 1)}"
            )
            ...
            if use_smote == "auto":
                ...
                print(
                    f"Автоматический выбор: {'ИСПОЛЬЗОВАТЬ SMOTE' if use_smote_final else 'НЕ использовать SMOTE'}"
                )
            else:
                use_smote_final = use_smote

            optimal_components = find_optimal_csp_components(
            ...
            train_bal_acc, train_acc, train_cm = csp.detailed_evaluation(
                X_train, y_train, "TRAIN"
            )
            val_bal_acc, val_acc, val_cm = csp.detailed_evaluation(
                X_val, y_val, "VALIDATION"
            )
            test_bal_acc, test_acc, test_cm = csp.detailed_evaluation(
                X_test, y_test, "TEST"
            )
            ...
            print(
                f"Статистики нормализации: mean={train_mean:.4f}, std={train_std:.4f}"
            )
            ...
            if use_autoencoder:
                ...
                autoencoder_val_loader = DataLoader(
                ...
                print(
                    "Использование Vision Transformer для классификации EEG..."
                )
                seq_length = end_idx - start_idx
                ...
                seq_length = end_idx - start_idx
                model = DeepConvNet(
                    input_channels=8, seq_length=seq_length, num_classes=2
                )
            ...
            predictions, true_labels, test_accuracy = test_model(
                model, test_loader
            )
            ...
            if use_autoencoder:
                embeddings_2d, all_labels = visualize_embeddings(
                    model, test_loader, title="Эмбеддинги EEG сигналов"
                )
